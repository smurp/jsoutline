...
var jsoutline = {
  ...
  traceAll: function(root, recurse, skipList, defaultName) {
    ...
    if (rootName) {
      if (!jsoutline.classes[rootName]) {
	jsoutline.classes[rootName] = {tracing: {}, skipping: {}};
      }
      for (i in tracingMethods) {
	  jsoutline.classes[rootName]['tracing'][tracingMethods[i]] = true;
      }
      for (i in skippingMethods) {
	  jsoutline.classes[rootName]['skipping'][skippingMethods[i]] = true;
      }
    }
